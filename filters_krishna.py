...
def smoothing(image, kernel_size=5):
    ...
    """
    smoothed_image = cv2.GaussianBlur(image, ksize=(
        kernel_size, kernel_size), sigmaX=0, sigmaY=0)
...
def sharpening(image, kernel_size=3, strength=1):
    ...
    """
    blurred = cv2.GaussianBlur(image, ksize=(
        kernel_size, kernel_size), sigmaX=0, sigmaY=0)
    sharpened = cv2.addWeighted(
        image, 1+(strength/10), blurred, -(strength/10), 0)
    return sharpened
